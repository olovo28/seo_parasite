...
- **Git: коммить после КАЖДОЙ завершённой задачи** (чтобы при необходимости легко откатиться). Если в одном запросе несколько логически разных задач — разбивай на отдельные коммиты на своё усмотрение. Сообщения нейтральные, по-человечески, **без следов ИИ и без `Co-Authored-By`**. **Перед каждым коммитом** убеждайся, что секреты не в индексе (`git ls-files --cached` не содержит `.env`, `data/`, `*.db`, кук, `reference/google_pro_1.4.3/`). `git push` — только по явному запросу (репозиторий локальный, без remote).
