...
- **Генерация через Claude API платная.** Разрешён ОДИН тест-прогон `npm run generate` (1 статья) для проверки — без спроса. **Массовое/батчи** (`generate-batch`, балк-генерация) — по запросу. Логику сперва проверяй детерминированно (импорт модулей, `node -e`, `npm test`); API зря не жги.
- **Боевой сайт — осторожно.** Разрешены публикация/удаление ОДНОЙ статьи для проверки — без спроса. **Массовая публикация/удаление** — по запросу. Публикация (`#article_publish`) и удаление (`POST /a/article/delete/{id}`) — реальные действия; профиль Dolphin — только с рабочей прокси.
...
- **Секреты не коммить.** Ключи Claude — в БД (`claude_keys`), токен Dolphin и пароль аккаунтов — в `.env`/`site_accounts`. В `.gitignore` (НЕ коммитим): БД `data/`, `.env`, `diagnostics-*.json`/`diagnostics/`, куки-сессии `reference/cookies/`, расширение YesCaptcha с вшитым ключом `reference/google_pro_1.4.3/`.
- **Все команды — из корня проекта.**
- **Git: коммить после КАЖДОЙ завершённой задачи** (чтобы при необходимости легко откатиться). Несколько логических задач в одном запросе → отдельные коммиты на своё усмотрение. Сообщения нейтральные, по-человечески, **без следов ИИ и без `Co-Authored-By`**. **Перед каждым коммитом** проверяй, что секреты не в индексе (`git ls-files --cached` без `.env`, `data/`, `*.db`, кук, `reference/google_pro_1.4.3/`). **`git push` — только по явному запросу** («пуш»/«запушь»): remote приватный `git@github.com:olovo28/seo_parasite.git`, ветка `main`, SSH-agent настроен → push без passphrase.
...
Цель — **высокий темп без потери качества**. Стандартный дев-цикл выполняй САМ, без подтверждений на каждом шаге, и отчитывайся итогом:
**правки кода → `npm test` (зелёный) → пересборка Docker → коммит**. Дроби на логические коммиты.

Спрашивай разрешение ТОЛЬКО на: **массовую/батч-генерацию** Claude API, **массовую** публикацию/удаление на боевом сайте, **`git push`**, и **необратимые/деструктивные** операции (force-push, удаление веток/данных, отправка данных вовне). Одиночный тест-прогон (1 генерация / 1 публикация / 1 удаление) — без спроса.

Правило «сначала оценка, потом правки» действует ТОЛЬКО для запросов вида «посмотри / изучи / что думаешь» — там сперва анализ, код не трогаем. Для чётко поставленной задачи — делаем сразу.

## Агенты и параллелизм (режим ultracode)

По умолчанию для КАЖДОЙ существенной задачи — оркестрация через **Workflow** (фазы understand → design → implement → review): параллельные субагенты на независимые части + **adversarial-верификация** находок/правок перед коммитом. Токены не экономим — приоритет максимальное качество и покрытие. Соло — только тривиальные/разговорные шаги и мелкие механические правки.

Где параллелить: ревью по измерениям (баги / перф / безопасность) с проверкой каждого finding скептиками; широкое исследование кода; реализация независимых правок (при правках в parallel — изоляция через worktree); «loop-until-dry» при поиске. Качество держим: `npm test` зелёный + ревью-агент(ы) перед коммитом, выводы агентов перепроверяем.
