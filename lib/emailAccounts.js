...
export function importEmailAccounts(db, text, { provider = 'gmx', country = 'at' } = {}) {
  ...
  return { added, skipped, errors, noProxy };
}
...
export function swapEmailProxy(db, id) {
  ...
  const url = assignUnusedProxy(db, { country });
  if (!url) return null;
