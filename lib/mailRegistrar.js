...
export async function createMailbox(db, { provider = 'gmx', country = null, enableImapAfter = true, onStep } = {}) {
  ...
  const c = String(country || process.env.GMX_PROXY_COUNTRY || 'at').trim().toLowerCase();
  const proxyUrl = assignProxy(db, { country: c, purpose: 'register' });
  if (!proxyUrl) throw new Error(`В пуле нет прокси для регистрации страны ${c} (раздел «Прокси»: группа с назначением «Регистрация»).`);
  ...
  try {
    ...
    db.prepare("INSERT OR IGNORE INTO email_accounts (provider, email, password, proxy, country, phone, status) VALUES (?, ?, ?, ?, ?, ?, 'verified')")
      .run(provider, res.email, res.password, proxyUrl, c, res.phone || null);
