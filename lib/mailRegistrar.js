...
export async function createMailbox(db, { provider = 'gmx', enableImapAfter = true, onStep } = {}) {
  ...
  const proxyUrl = assignProxy(db, { country: process.env.GMX_PROXY_COUNTRY || 'at', purpose: 'register' });
  if (!proxyUrl) throw new Error('В пуле нет прокси для регистрации нужной страны (раздел «Прокси»: группа с назначением «Регистрация»).');
