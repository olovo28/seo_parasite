...
export function updateRegistration(db, id, fields = {}) {
  const allowed = ['status', 'confirm_url', 'next_check_at', 'checks', 'account_id', 'error', 'site_username', 'site_password', 'identity', 'submitted_at', 'approved_at', 'last_checked_at', 'warm_visits', 'warm_target', 'next_warm_at', 'warm_cookies'];
  const sets = [];
  const vals = [];
...
// Регистрации, которым подошло время очередного визита прогрева (для планировщика).
export function dueWarmings(db, nowUtc) {
  return db
    .prepare(
      `SELECT r.id FROM site_registrations r JOIN sites s ON s.id = r.site_id
       WHERE r.status = 'warming' AND r.next_warm_at IS NOT NULL AND r.next_warm_at <= ? AND s.active = 1
       ORDER BY r.next_warm_at, r.id`,
    )
    .all(nowUtc);
}
