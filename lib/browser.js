...
import { getDolphinConfig } from './farmConfig.js';

// Опции старта профиля из «Настроек фарма» (режимы отпечатка + headless). Best-effort: на сбой — пустой конфиг (дефолты).
function dolphinLaunchOpts() {
  try {
    const c = getDolphinConfig(getDb());
    return { config: { canvas: c.canvas, webgl: c.webgl, clientRect: c.clientRect, mediaDevices: c.mediaDevices, webrtc: c.webrtc }, headless: c.headless };
  } catch {
    return {};
  }
}
...
export async function launchProfileClean(profileName) {
  ...
  let browser;
  try {
    ({ browser } = await ensureProfileAndLaunch({ name: profileName, ...dolphinLaunchOpts() }));
...
export async function launchProfileWithProxy({ proxy }) {
  ...
  try {
    ...
    ({ browser, profileId } = await ensureProfileAndLaunch({ name, proxyBase: proxy, maxPortTries: 1, ...dolphinLaunchOpts() }));
    const page = await cleanStart(browser);
...
export async function launchForAccount({ profileName, proxy, action, log = console.log, tries = LAUNCH_TRIES } = {}) {
  if (proxy) {
    ...
    if (last) last.proxyFailed = true; // прокси аккаунта не поднялась — вызывающий может подменить её из пула
    throw last;
  }
