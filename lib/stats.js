...
export async function collectArticleStats(db, articleId, { reason = 'manual', onStep } = {}) {
  ...
  // HTTP-first: без Dolphin, если адаптер умеет и есть сессия (cookies+proxy). needLogin → фолбэк на профиль.
  if (adapter.fetchArticleStatsHttp && acc.cookies && proxy) {
    try {
      const stats = await adapter.fetchArticleStatsHttp({ origin: site.origin, siteArticleId, cookies: acc.cookies, proxy, log });
      saveStatsSnapshot(db, articleId, stats, { reason });
      log(`#${articleId}: ${stats.totalViews} просмотров (из поиска ${stats.channels?.seo ?? 0}), перцентиль ${stats.percentile ?? '-'} [HTTP].`);
      return stats;
    } catch (e) {
      if (!e.needLogin && !e.needBrowser) throw e;
      log(e.needBrowser ? 'Нет согласия кокпита — собираю через профиль Dolphin.' : 'HTTP-сессия истекла — собираю через профиль Dolphin.');
    }
  }
...
export async function collectStatsForSite(db, siteId, { reason = 'daily', onStep } = {}) {
  ...
  for (const [, group] of byAccount) {
    ...
    // HTTP-first: собираем что можем по HTTP (без Dolphin); при истёкшей сессии — оставшиеся через профиль.
    let toBrowser = group;
    if (adapter.fetchArticleStatsHttp && acc.cookies && proxy) {
      const rest = [];
      let dead = false;
      for (const a of group) {
        if (dead) { rest.push(a); continue; }
        const siteArticleId = adapter.parseSiteArticleId(a.site_url);
        if (!siteArticleId) { fail += 1; continue; }
        try {
          const stats = await adapter.fetchArticleStatsHttp({ origin: site.origin, siteArticleId, cookies: acc.cookies, proxy, log });
          saveStatsSnapshot(db, a.id, stats, { reason });
          ok += 1;
        } catch (e) {
          if (e.needLogin || e.needBrowser) { dead = true; rest.push(a); }
          else { fail += 1; log(`#${a.id}: HTTP-стата — ${e.message}`); }
        }
      }
      if (!rest.length) { log(`Аккаунт ${acc.label || acc.username}: собрано по HTTP.`); continue; }
      log(`Аккаунт ${acc.label || acc.username}: HTTP-сессия истекла — добираю ${rest.length} через профиль…`);
      toBrowser = rest;
    }
    ...
    try {
      ...
      // не удалось даже залогиниться/запустить профиль — оставшиеся статьи аккаунта в fail
      fail += toBrowser.length;
      if (e.banned) {
