...
async function signup(page, { identity, solver, smsProvider, proxy, domain = process.env.GMX_SIGNUP_DOMAIN || 'gmx.at', log = console.log } = {}) {
  if (!solver?.solveCaptchaFox) throw new Error('Для регистрации GMX нужен сервис CaptchaFox (2captcha).');
  if (!smsProvider) throw new Error('Для регистрации GMX нужен SMS-сервис (5sim).');
...
async function enableImap(page, { log = console.log } = {}) {
  const url = process.env.GMX_IMAP_SETTINGS_URL || 'https://account.gmx.net/account/security/pop-imap';
