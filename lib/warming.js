...
const WARM_TARGET = Number(process.env.WARM_TARGET_VISITS || 3); // визитов (≈дней) прогрева по умолчанию
const WARM_MIN_H = Number(process.env.WARM_MIN_HOURS || 18); // мин. интервал до следующего визита
const WARM_MAX_H = Number(process.env.WARM_MAX_HOURS || 30); // макс. интервал (нерегулярный ритм — не пачками)
const WARM_MIN_PAGES = Number(process.env.WARM_MIN_PAGES || 5);
const WARM_MAX_PAGES = Number(process.env.WARM_MAX_PAGES || 20);

// Нерегулярное следующее время визита: сейчас + случайно 18..30 ч (≈раз в сутки, но со сдвигом).
function nextWarmStamp() {
  const h = WARM_MIN_H + Math.random() * (WARM_MAX_H - WARM_MIN_H);
...
export function startWarming(db, { siteId, emailAccountId, identity, siteUsername, sitePassword, target } = {}) {
  ...
  updateRegistration(db, rid, { status: 'warming', warm_visits: 0, warm_target: target || WARM_TARGET, next_warm_at: utcStamp(), error: null });
  logRegEvent(db, rid, 'warm_start', `Начат прогрев: цель ${target || WARM_TARGET} визитов.`);
  return rid;
}
...
export async function warmVisit(db, { registrationId, onStep } = {}) {
  ...
  const target = reg.warm_target || WARM_TARGET;
  ...
  try {
    ...
    const visited = await humanBrowse(page, { origin: site.origin, minPages: WARM_MIN_PAGES, maxPages: WARM_MAX_PAGES, log });
    ...
    updateRegistration(db, reg.id, {
      ...
      next_warm_at: done ? null : nextWarmStamp(),
