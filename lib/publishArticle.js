...
import { injectBinomLinks, insertLinkBlockMulti } from './linkblock.js';
...
export async function publishArticleById(db, articleId, { accountId, onStep } = {}) {
  ...
  // Блок ссылок подставляем ПРИ ПУБЛИКАЦИИ (актуальный блок), а не вшитый при генерации (links_pending=1).
  // Правка/отключение блока сразу отражается на черновиках. Легаси (links_pending=0) — тело уже со вшитым блоком.
  let bodyHtml = article.body_html;
  if (article.links_pending && article.link_block_id) {
    const blk = db.prepare('SELECT * FROM link_blocks WHERE id = ?').get(article.link_block_id);
    if (blk && blk.enabled && (blk.block || '').trim()) {
      const inj = injectBinomLinks(blk.block, { articleParam: site.binom_param_article, linkParam: site.binom_param_link, trackingId: article.tracking_id });
      if (bodyHtml.includes(LINK_MARKER)) {
        bodyHtml = bodyHtml.split(LINK_MARKER).join(inj.block);
      } else {
        const positions = String(blk.link_position || '1').split(',').map((s) => s.trim()).filter(Boolean);
        bodyHtml = insertLinkBlockMulti(bodyHtml, inj.block, positions.length ? positions : ['1']);
      }
      // Пересобираем article_links под текущий блок (для Binom-трекинга).
      db.prepare('DELETE FROM article_links WHERE article_id = ?').run(articleId);
      const insL = db.prepare('INSERT INTO article_links (article_id, link_id, anchor, base_url, final_url) VALUES (?, ?, ?, ?, ?)');
      for (const l of inj.links) insL.run(articleId, l.link_id, l.anchor, l.base_url, l.final_url);
      step(`Блок ссылок подставлен при публикации (ссылок: ${inj.links.length}).`);
    } else {
      bodyHtml = bodyHtml.split(LINK_MARKER).join(''); // блок отключён/удалён → публикуем без ссылок
      step('Блок ссылок отключён/не найден — публикую без ссылок.');
    }
  }
  ...
  try {
    const res = await withProxySwap(db, acc, (px) => publishArticleToSite({
      ...
      proxy: px,
      article: { title: article.title, body_html: bodyHtml, tags },
      cookies: acc.cookies,
    ...
    markFailed(e.message);
    // Аккаунт забанен сайтом — отключаем, чтобы не долбиться в него бесконечно (публикация/логин им прекращаются).
    if (e.banned && acc?.id) {
      db.prepare('UPDATE site_accounts SET enabled = 0 WHERE id = ?').run(acc.id);
      step(`Аккаунт ${acc.label} ЗАБАНЕН сайтом — отключил его (больше не публикуем и не логинимся им).`);
    }
    e.handled = true;
    throw e;
  }
...
export async function deleteArticlesGrouped(db, articleIds, { concurrency = 5, delayMs = 10000, clearDeleteAtOnFail = false, shouldStop = () => false, onStep } = {}) {
  ...
  await runPool(groupArr, concurrency, async (g) => {
    ...
    try {
      ...
      if (e.banned) {
        db.prepare('UPDATE site_accounts SET enabled = 0 WHERE id = ?').run(g.acc.id);
        step(`[${accName}] аккаунт ЗАБАНЕН сайтом — отключил его.`);
      }
