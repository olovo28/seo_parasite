...
import { publishArticleToSite, deleteArticleFromSite, deleteManyInSession } from './publisher.js';
...
export async function publishArticleById(db, articleId, { accountId, onStep } = {}) {
  ...
  try {
    const res = await publishArticleToSite({
...
export async function deleteArticleFromSiteById(db, articleId, { accountId, onStep } = {}) {
  ...
  try {
    const res = await deleteArticleFromSite({
      adapter,
      profileName: site.profile_name,
      origin: site.origin,
      credentials: { username: acc.username, password: acc.password },
...
export async function deleteArticlesGrouped(db, articleIds, { concurrency = 5, delayMs = 10000, clearDeleteAtOnFail = false, onStep } = {}) {
  const step = (m) => {
    console.log(m);
    if (onStep) onStep(m);
  };
  ...
  await runPool(groupArr, concurrency, async (g) => {
    ...
    step(`[${accName}] ${g.items.length} статей — поднимаю профиль…`);
    ...
    try {
      await deleteManyInSession({
        ...
        log: step,
        onResult: (it, res) => {
          const msg = `[аккаунт ${accName}] ${res.message}`;
          logp.run(it.articleId, res.ok ? 1 : 0, msg);
          if (res.ok) {
      ...
      // сессия/профиль не поднялись даже с ретраями → все статьи аккаунта в ошибку
  ...
  step(`Готово: снято ${ok}, ошибок ${fail} из ${articleIds.length}.`);
  return { total: articleIds.length, ok, fail };
}
