...
function pickProxy(db, { country, purpose = null, siteId = null, excludeAssignedEmails = false } = {}) {
  ...
  if (purpose) {
    // Прокси без группы (group_id NULL) = универсальная (любое назначение); в группе — только если назначение указано.
    where.push("(p.group_id IS NULL OR (',' || COALESCE(g.purposes,'') || ',') LIKE @purpose)");
    params.purpose = `%,${purpose},%`;
  }
  if (siteId != null) {
    // Без группы = все сайты; группа без привязки (site_ids пусто/NULL) = все сайты; иначе siteId должен входить.
    where.push("(p.group_id IS NULL OR g.site_ids IS NULL OR g.site_ids = '' OR (',' || g.site_ids || ',') LIKE @site)");
...
// Выдать ЗАМЕНУ для публикации аккаунта: прокси из групп «Публикация» этого сайта, которая
//  (а) НЕ закреплена ни за каким другим аккаунтом (site_accounts) и ни за одной почтой (email_accounts) → нет пересечения IP;
//  (б) простаивала ≥ idleHours (last_assigned_at пусто или старше) → LRU;
//  (в) не равна текущей (отвалившейся) прокси аккаунта.
// Страну для публикации НЕ фильтруем (её задаёт состав группы). Помечает выдачу. null — если свободной нет.
export function assignPublishProxy(db, { siteId = null, accountId = null, excludeUrl = null, idleHours = 24 } = {}) {
  const where = [
    "(p.group_id IS NULL OR (',' || COALESCE(g.purposes,'') || ',') LIKE '%,publish,%')",
    "(p.group_id IS NULL OR g.site_ids IS NULL OR g.site_ids = '' OR (',' || g.site_ids || ',') LIKE @site)",
    "(p.last_assigned_at IS NULL OR p.last_assigned_at < datetime('now', @idle))",
    "p.url NOT IN (SELECT proxy FROM email_accounts WHERE proxy IS NOT NULL)",
    "p.url NOT IN (SELECT proxy FROM site_accounts WHERE proxy IS NOT NULL AND id != @acc)",
    "(@exclude IS NULL OR p.url != @exclude)",
  ];
  const params = {
    site: `%,${Number(siteId)},%`,
    idle: `-${Math.max(1, Number(idleHours) || 24)} hours`,
    acc: accountId == null ? -1 : Number(accountId),
    exclude: excludeUrl || null,
  };
  const sql = `SELECT p.id, p.url FROM proxies p LEFT JOIN proxy_groups g ON g.id = p.group_id
    WHERE ${where.join(' AND ')}
    ORDER BY (p.last_assigned_at IS NOT NULL), p.last_assigned_at ASC, RANDOM() LIMIT 1`;
...
// Подменить прокси аккаунта публикации (когда текущая отвалилась) на свободную из пула «Публикация». Возвращает url или null.
export function swapSiteAccountProxy(db, accountId) {
  const acc = db.prepare('SELECT id, site_id, proxy FROM site_accounts WHERE id = ?').get(accountId);
  if (!acc) return null;
  const url = assignPublishProxy(db, { siteId: acc.site_id, accountId: acc.id, excludeUrl: acc.proxy });
  if (!url) return null;
  db.prepare('UPDATE site_accounts SET proxy = ? WHERE id = ?').run(url, accountId);
  return url;
}
