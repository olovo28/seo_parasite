...
// Проверка одобрения админом: дважды в день (каждые 12 ч) в течение 30 дней → потом «не одобрено».
const APPROVAL_CHECK_INTERVAL_MS = Number(process.env.REGISTER_CHECK_INTERVAL_MS || 12 * 3600 * 1000); // 12 ч = дважды в день
export const MAX_APPROVAL_CHECKS = Number(process.env.REGISTER_MAX_CHECKS || 60); // 2/день × 30 дней
