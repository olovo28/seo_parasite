...
import { humanBrowse, grantSitePermissions } from './humanize.js';
...
export async function registerOnSite(db, { siteId, emailAccountId, onStep } = {}) {
  ...
  try {
    ...
    await grantSitePermissions(page, site.origin).catch(() => {}); // разрешаем уведомления/гео (нативный запрос не висит)
    ...
    const farm = getFarmConfig(db);
    if (farm.register_humanize) {
      try {
      ...
      await humanBrowse(page, { origin: site.origin, minPages: farm.register_browse_min, maxPages: farm.register_browse_max, entry: farm.warm_entry, pushSubscribe: farm.push_subscribe, log }).catch((e) => log(`Обзор перед регистрацией пропущен: ${e.message}`));
    }
...
export async function checkApproval(db, { registrationId, onStep } = {}) {
  ...
  // Без прокси IMAP GMX отклоняет логин с чужого IP — даём ПОНЯТНУЮ ошибку, а не «LOGIN отклонён».
  if (!emailAcc.proxy) {
    const m = 'У почты не задана прокси — GMX блокирует IMAP с не-AT IP. Задай AT-прокси почте (раздел «Почты» или переоткрой добавление).';
    updateRegistration(db, reg.id, { last_checked_at: utcStamp(), next_check_at: utcStamp(new Date(Date.now() + APPROVAL_CHECK_INTERVAL_MS)), error: m });
    logRegEvent(db, reg.id, 'approval_check', m);
    return { ok: false, status: reg.status, message: m };
  }
