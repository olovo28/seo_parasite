...
import { launchProfileWithProxy, cleanupProfile, screenshotTo } from './browser.js';
...
const MAX_APPROVAL_CHECKS = Number(process.env.REGISTER_MAX_CHECKS || 7); // ~неделя ежедневных проверок
...
export async function registerOnSite(db, { siteId, emailAccountId, onStep } = {}) {
  ...
  try {
    ...
    updateRegistration(db, reg.id, { status: 'awaiting_admin', confirm_url: confirmUrl, submitted_at: reg.submitted_at || utcStamp(), next_check_at: utcStamp(new Date(Date.now() + DAY_MS)), error: null });
...
export async function checkApproval(db, { registrationId, onStep } = {}) {
  ...
  try {
    ...
    updateRegistration(db, reg.id, { next_check_at: utcStamp(new Date(Date.now() + DAY_MS)), error: `IMAP: ${e.message}` });
    return { ok: false, status: 'awaiting_admin', message: `IMAP недоступен: ${e.message}. Перенёс проверку на сутки.` };
  }
  ...
  if (checks >= MAX_APPROVAL_CHECKS) {
    updateRegistration(db, reg.id, { status: 'failed', checks, error: `Не одобрено за ${checks} проверок.` });
    return { ok: false, status: 'failed', message: `Не одобрено за ${checks} проверок — остановлено.` };
  }
  updateRegistration(db, reg.id, { checks, next_check_at: utcStamp(new Date(Date.now() + DAY_MS)) });
  return { ok: false, status: 'awaiting_admin', message: `Письма об одобрении ещё нет (проверка ${checks}/${MAX_APPROVAL_CHECKS}). Следующая через сутки.` };
}
