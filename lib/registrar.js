...
export async function registerOnSite(db, { siteId, emailAccountId, onStep } = {}) {
  ...
  try {
    ...
    if (farm.register_humanize) {
      ...
      await humanBrowse(page, { origin: site.origin, minPages: farm.register_browse_min, maxPages: farm.register_browse_max, log }).catch((e) => log(`Обзор перед регистрацией пропущен: ${e.message}`));
    }
