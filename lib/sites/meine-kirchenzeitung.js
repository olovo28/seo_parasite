// Адаптер сайта meine-kirchenzeitung.de: логин, публикация, удаление, регистрация, формат тела.
// Та же CMS-платформа, что meinbezirk, но отдельный самостоятельный модуль (свои нюансы формы
// регистрации: ТРИ обязательных чекбокса согласия tos/dps/license, свой набор городов, свой домен писем).
...
const CATEGORY = '18'; // NOTE: id категории публикации может отличаться на .de — уточнить при первой публикации.
...
const REGISTER_PATH = process.env.MEINE_KIRCHENZEITUNG_REGISTER_PATH || '/register';
...
// Одна попытка регистрации: заполнить форму, решить капчу, отправить.
async function attemptRegister(page, { origin, identity, email, password, solver, log = console.log } = {}, attempt = 1) {
  ...
  // Локация: на этой форме свои id городов (DE) — австрийский id личности не подходит, поэтому
  // выбираем первую валидную опцию селекта (не плейсхолдер).
  await page.evaluate(() => {
    const sel = document.querySelector('#register_location');
    if (!sel) return;
  ...
  // Согласия: на этой форме ТРИ обязательных чекбокса (tos/dps/license) — отмечаем все.
  await page.evaluate(() => {
...
// Письмо подтверждения регистрации (домен бренда сайта + общая инфраструктура сети).
const confirmationEmail = { fromMatch: /meine-kirchenzeitung|regionalmedien/i, subjectMatch: /(Best[äa]tig|Registrier|Aktivier|willkommen|E-?Mail)/i };

function extractConfirmUrl(links) {
  const arr = Array.isArray(links) ? links : [];
  return (
    arr.find((u) => /meine-kirchenzeitung|regionalmedien/i.test(u) && /(confirm|activat|aktivier|bestaetig|best%C3%A4tig|registrier|token=|code=|hash=)/i.test(u)) || null
  );
}
...
const approvalEmail = { fromMatch: /meine-kirchenzeitung|regionalmedien/i, subjectMatch: /(freigeschalt|freigegeb|aktiviert|genehmigt|willkommen|best[äa]tigt|Community)/i };
...
export default {
  name: 'meine-kirchenzeitung',
  label: 'meine-kirchenzeitung.de',
  login,
  isLoggedIn,
  publish,
  deleteArticle,
