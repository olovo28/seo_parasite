...
import { proxyFetch, isLoginResponse, mergeSetCookies, cookieHeader } from '../httpSession.js';
...
// ===== HTTP-транспорт (БЕЗ Dolphin): голые запросы через прокси аккаунта с его cookies =====
// Доказано зондом: meinbezirk принимает кука-авторизацию от не-браузерного клиента; анти-бота нет.

// HTTP-сбор статистики: GET кокпита → cockpitData прямо в HTML → parseCockpitStats. needLogin → фолбэк на браузер.
async function fetchArticleStatsHttp({ origin, siteArticleId, cookies, proxy, log = console.log } = {}) {
  if (!siteArticleId) throw new Error('Нет siteArticleId — неоткуда брать статистику.');
  log(`HTTP-статистика кокпита (article ${siteArticleId})…`);
  const r = await proxyFetch(`${origin}/cockpit/contentcockpit/article/${siteArticleId}`, { proxy, cookies, origin });
  if (isLoginResponse(r)) {
    const e = new Error('Кокпит увёл на /login — сессия истекла (HTTP).');
    e.needLogin = true;
    throw e;
  }
  try {
    return parseCockpitStats(r.body);
  } catch (e) {
    // Сессия живая, но нет cockpitData → не принято согласие кокпита (Google Charts). По HTTP его не выставить —
    // помечаем needBrowser, чтобы сборщик добрал через Dolphin (он примет согласие и пересохранит куки).
    if (!e.needLogin) e.needBrowser = true;
    throw e;
  }
}

// HTTP-публикация: GET формы → CSRF-токен → POST Symfony-формы. Возвращает { ok, url?, message }.
async function publishHttp({ origin, article, cookies, proxy, log = console.log } = {}) {
  log('HTTP-публикация: получаю форму и CSRF-токен…');
  const g = await proxyFetch(`${origin}/a/article/new`, { proxy, cookies, origin });
  if (isLoginResponse(g)) {
    const e = new Error('Форма публикации ушла на /login — сессия истекла (HTTP).');
    e.needLogin = true;
    throw e;
  }
  const token = (g.body.match(/name="article\[_token\]"\s+value="([^"]+)"/) || [])[1];
  if (!token) throw new Error('В форме не найден article[_token] (HTTP-публикация невозможна).');
  // GET формы мог обновить сессионную/CSRF-куку через Set-Cookie — POST должен идти С НЕЙ, иначе токен «недействителен».
  const postCookies = mergeSetCookies(cookieHeader(cookies, new URL(origin).host), g.setCookie);
  const tags = (article.tags || []).filter(Boolean);
  const p = new URLSearchParams();
  p.set('article[_token]', token);
  p.set('article[title]', article.title || '');
  p.set('article[category]', CATEGORY);
  p.set('article[content_text]', htmlToBBCode(article.body_html));
  p.set('article[tag_name_list]', tags.join(','));
  p.set('article[likes_enabled]', '1');
  p.set('article[publish]', ''); // нажата кнопка «Veröffentlichen & anzeigen»
  log(`HTTP-публикация: отправляю форму (тегов ${tags.length})…`);
  const res = await proxyFetch(`${origin}/a/article/new`, { proxy, cookies: postCookies, origin, method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: p.toString() });
  if (isLoginResponse(res)) {
    const e = new Error('Во время HTTP-публикации выбросило на /login.');
    e.needLogin = true;
    throw e;
  }
  if (res.status >= 300 && res.status < 400 && res.location && !/\/a\/article\/new/.test(res.location)) {
    const url = res.location.startsWith('http') ? res.location : origin + res.location;
    return { ok: true, url, message: `Опубликовано (HTTP). URL: ${url}` };
  }
  const err = (res.body.match(/<(?:span|div|li|p)[^>]*class="[^"]*(?:form-error|callout alert|flashmessage-box|help-text error)[^"]*"[^>]*>([\s\S]{0,200}?)<\//i) || [])[1];
  return { ok: false, message: `HTTP-публикация не прошла (HTTP ${res.status}${res.location ? ' → ' + res.location : ''})${err ? ': ' + err.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : ' — осталась на форме (валидация?)'}` };
}

// HTTP-удаление: POST /a/article/delete/{id} с куками (без токена). Возвращает { ok, message }.
async function deleteArticleHttp({ origin, siteArticleId, cookies, proxy, log = console.log } = {}) {
  log(`HTTP-удаление (article ${siteArticleId})…`);
  const r = await proxyFetch(`${origin}/a/article/delete/${siteArticleId}`, { proxy, cookies, origin, method: 'POST', headers: { 'x-requested-with': 'XMLHttpRequest' } });
  if (isLoginResponse(r) || r.status === 401 || r.status === 403) {
    const e = new Error(`HTTP-удаление отклонено — нет сессии (HTTP ${r.status}).`);
    e.needLogin = true;
    throw e;
  }
  if (r.status === 200 || r.status === 204) return { ok: true, message: `Удалено с сайта (HTTP ${r.status}, article ${siteArticleId}).` };
  return { ok: false, message: `Сайт ответил HTTP ${r.status} на удаление ${siteArticleId} (нет прав / уже удалена?).` };
}

// HTTP-логин (БЕЗ Dolphin): GET /login → _csrf_token → POST формы Symfony. Возвращает { ok, cookies, message }
// (cookies — массив в CDP-формате для сохранения у аккаунта; на бане кидает e.banned). needLogin не нужен.
async function loginHttp({ origin, username, password, cookies, proxy, log = console.log } = {}) {
  if (!username || !password) throw new Error('Нет кредов аккаунта (username/password).');
  log(`HTTP-логин (${username})…`);
  const host = new URL(origin).host;
  const g = await proxyFetch(`${origin}/login`, { proxy, cookies, origin });
  const token = (g.body.match(/name="_csrf_token"\s+value="([^"]+)"/) || [])[1];
  const postCookies = mergeSetCookies(cookieHeader(cookies, host), g.setCookie); // GET ставит сессионную/CSRF-куку
  const p = new URLSearchParams();
  p.set('_username', username);
  p.set('_password', password);
  p.set('_remember_me', 'on');
  if (token) p.set('_csrf_token', token);
  const res = await proxyFetch(`${origin}/login`, { proxy, cookies: postCookies, origin, method: 'POST', headers: { 'content-type': 'application/x-www-form-urlencoded' }, body: p.toString() });
  const finalHeader = mergeSetCookies(postCookies, res.setCookie); // итоговые куки авторизованной сессии
  if (res.status >= 300 && res.status < 400 && !/\/login/.test(res.location || '')) {
    const root = '.' + host.split('.').slice(-2).join('.');
    const cookieArr = finalHeader
      .split('; ')
      .filter(Boolean)
      .map((kv) => { const i = kv.indexOf('='); return { name: kv.slice(0, i), value: kv.slice(i + 1), domain: root, path: '/' }; });
    return { ok: true, cookies: cookieArr, message: `HTTP-логин успешен (${cookieArr.length} cookies).` };
  }
  const flash = (res.body.match(/class="[^"]*(?:flashmessage-box|callout|alert)[^"]*"[^>]*>([\s\S]{0,200}?)</i) || [])[1];
  const msg = flash ? flash.replace(/<[^>]+>/g, ' ').replace(/\s+/g, ' ').trim() : `HTTP-логин не удался (HTTP ${res.status})`;
  const e = new Error(msg);
  if (/gesperrt|blocked|disabled|suspend/i.test(msg)) e.banned = true;
  throw e;
}
