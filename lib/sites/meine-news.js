// Адаптер сайта meine-news.de: логин, публикация, удаление, регистрация, формат тела.
// Та же CMS-платформа, что meinbezirk, но отдельный самостоятельный модуль (свои нюансы формы
// регистрации: ДВА обязательных чекбокса согласия tos/dps, свой набор городов, свой домен писем).
...
const REGISTER_PATH = process.env.MEINE_NEWS_REGISTER_PATH || '/register';
...
async function attemptRegister(page, { origin, identity, email, password, solver, log = console.log } = {}, attempt = 1) {
  ...
  // Согласия: отмечаем ВСЕ обязательные чекбоксы согласия (на этой форме tos/dps).
  await page.evaluate(() => {
...
const confirmationEmail = { fromMatch: /meine-news|regionalmedien/i, subjectMatch: /(Best[äa]tig|Registrier|Aktivier|willkommen|E-?Mail)/i };

function extractConfirmUrl(links) {
  const arr = Array.isArray(links) ? links : [];
  return (
    arr.find((u) => /meine-news|regionalmedien/i.test(u) && /(confirm|activat|aktivier|bestaetig|best%C3%A4tig|registrier|token=|code=|hash=)/i.test(u)) || null
  );
}
...
const approvalEmail = { fromMatch: /meine-news|regionalmedien/i, subjectMatch: /(freigeschalt|freigegeb|aktiviert|genehmigt|willkommen|best[äa]tigt|Community)/i };
