...
// Ошибка похожа на проблему прокси? Помимо e.proxyFailed (провал гейта при запуске) ловим и браузерные/сетевые
// сбои прокси-туннеля В ПРОЦЕССЕ (ERR_TUNNEL_CONNECTION_FAILED и т.п.) — тогда тоже свапаем прокси и повторяем.
export function isProxyError(e) {
  const m = String(e?.message || '');
  return !!e?.proxyFailed || /ERR_TUNNEL_CONNECTION_FAILED|ERR_PROXY_CONNECTION_FAILED|ERR_(CONNECTION_(RESET|CLOSED|REFUSED|TIMED_OUT|ABORTED)|TIMED_OUT|SOCKS_CONNECTION_FAILED|EMPTY_RESPONSE)|net::ERR_|ECONNRESET|ETIMEDOUT|EAI_AGAIN|\btunnel\b|fetch failed|socket hang up/i.test(m);
}

// Выполнить операцию аккаунта с ЕГО прокси (sticky). Если прокси НЕ поднялась/отвалилась (isProxyError) — подменить
// её на свободную из пула «Публикация» (≥24ч простоя, не занята другими аккаунтами/почтами) и повторить ОДИН раз.
