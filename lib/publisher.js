...
export async function publishArticleToSite({ adapter, profileName, origin, credentials, proxy, article, cookies, onCookies, log = console.log }) {
  // HTTP-first: если адаптер умеет публиковать по HTTP и есть сохранённая сессия (cookies+proxy) — без Dolphin.
  if (adapter.publishHttp && Array.isArray(cookies) && cookies.length && proxy?.host) {
    try {
      const r = await adapter.publishHttp({ origin, article, cookies, proxy, log });
      if (r?.ok) return r;
      if (!r?.needLogin) return r; // обычная ошибка (валидация формы) — отдаём как есть, Dolphin не нужен
    } catch (e) {
      if (e.needLogin) log('HTTP-сессия истекла — публикую через профиль Dolphin (перелогин).');
      else log(`HTTP-публикация не удалась (${e.message}) — пробую через профиль Dolphin.`);
    }
  }
  let browser;
  let page;
...
export async function deleteArticleFromSite({ adapter, profileName, origin, credentials, proxy, siteArticleId, cookies, onCookies, onStats, log = console.log }) {
  // HTTP-first: удаление без Dolphin, если адаптер умеет и есть сессия (cookies+proxy).
  if (adapter.deleteArticleHttp && Array.isArray(cookies) && cookies.length && proxy?.host) {
    try {
      return await deleteOneHttp({ adapter, origin, cookies, proxy, siteArticleId, onStats, log });
    } catch (e) {
      if (e.needLogin) log('HTTP-сессия истекла — удаляю через профиль Dolphin.');
      else log(`HTTP-удаление не удалось (${e.message}) — через профиль Dolphin.`);
    }
  }
  let browser;
  let page;
