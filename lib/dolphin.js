...
export async function createProfile({ name, proxy, config = {} }) {
  ...
  const body = {
    ...
    webrtc: { mode: config.webrtc || 'altered' },
    // canvas/webgl/clientRect/mediaDevices — режим из «Настроек фарма» (дефолт noise/маска: профили с одной машины
    // не должны склеиваться по «железу»; уникальный шум на каждый профиль).
    canvas: { mode: config.canvas || 'noise' },
    webgl: { mode: config.webgl || 'noise' },
    webglInfo: { mode: 'manual', vendor: fp.webgl.unmaskedVendor, renderer: fp.webgl.unmaskedRenderer },
    clientRect: { mode: config.clientRect || 'noise' },
    timezone: { mode: 'auto' },
    locale: { mode: 'auto' },
    ...
    mediaDevices: config.mediaDevices === 'real' ? { mode: 'real' } : { mode: 'manual', audioInputs: 1, videoInputs: 1, audioOutputs: 1 },
    ports: { mode: 'protect' },
    doNotTrack: false,
  };
...
export async function startProfile(id, { retryIfRunning = true, headless } = {}) {
  // headless из «Настроек фарма» (или DOLPHIN_HEADLESS=1) — старт без окна; фингерпринт спуфится так же.
  const useHeadless = headless != null ? !!headless : process.env.DOLPHIN_HEADLESS === '1';
  const hl = useHeadless ? '&headless=1' : '';
  const res = await fetchT(`${LOCAL_API}/browser_profiles/${id}/start?automation=1${hl}`, {}, 90000);
  ...
  if (!res.ok && retryIfRunning && /already running|E_BROWSER_RUN_DUPLICATE/i.test(text)) {
    ...
    return startProfile(id, { retryIfRunning: false, headless });
  }
...
export async function ensureProfileAndLaunch({ name, proxyBase, maxPortTries = 7, config = {}, headless } = {}) {
  ...
  if (existing) {
    ...
    const automation = await startProfile(existing.id, { headless });
  ...
  for (let port = startPort; port <= lastPort; port++) {
    ...
    let id;
    try {
      id = await createProfile({ name, proxy, config });
      console.log(`  Порт ${port}: профиль создан (id=${id}), запускаю...`);
      const automation = await startProfile(id, { headless });
      const browser = await connect(automation);
