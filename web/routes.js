...
<div class="d-flex flex-wrap gap-2"><button type="submit" class="btn btn-primary">Сохранить</button><button type="submit" formaction="/sites/${id}/toggle" class="btn btn-outline-secondary">${s.active ? 'Выключить' : 'Включить'}</button></div>
    ...
    const accRows = siteAccs
      .map(
        (acc) => `<tr><td>${acc.id}</td><td>${esc(acc.username)}</td><td class="text-secondary mono copy-pw" data-pw="${esc(acc.password)}" title="клик — скопировать пароль аккаунта" style="cursor:pointer">${esc(mask(acc.password))}</td><td class="text-secondary mono">${esc((acc.proxy || '-').split(':')[0])}</td><td>${esc(acc.label || '-')}</td><td>${acc.enabled ? '<span class="badge bg-green text-white">вкл</span>' : '<span class="badge bg-secondary text-white">выкл</span>'}</td>
