...
<td><div class="d-flex gap-1"><a href="/prompts/${p.id}" class="btn btn-sm btn-outline-secondary">ред.</a>${hidden ? `<form method="post" action="/prompts/${p.id}/unhide"><button class="btn btn-sm btn-outline-success">вернуть</button></form>` : `<form method="post" action="/prompts/${p.id}/delete" onsubmit="return confirm('Скрыть промт из списка? (статьи по нему не пострадают)')"><button class="btn btn-sm btn-outline-danger">скрыть</button></form>`}</div></td></tr>`;
    ...
    const defBackend = process.env.CLAUDE_BACKEND === 'cli' ? 'cli' : 'api';
    // Выбор движка генерации: API (платно) или подписочный CLI (в рамках тарифа). Пачка — только API.
    const backendRadio = (name = 'backend') => `<div class="col-auto"><label class="form-label d-block mb-1">Движок</label>
<label class="form-check form-check-inline mb-0"><input class="form-check-input" type="radio" name="${name}" value="api"${defBackend === 'api' ? ' checked' : ''}><span class="form-check-label">API <span class="text-secondary">(платно)</span></span></label>
<label class="form-check form-check-inline mb-0"><input class="form-check-input" type="radio" name="${name}" value="cli"${defBackend === 'cli' ? ' checked' : ''}><span class="form-check-label">Тариф <span class="text-secondary">(подписка)</span></span></label></div>`;
