...
import { createList, removeList, getList, listLists, listItems, getItem, addKeywordsToList, addManualKeywordsToList, setItemStatus, linkItemArticle, removeItem, ITEM_STATUSES } from '../lib/lists.js';
...
import { PROSPECT_STATUSES, PROSPECT_ENGINES, DISCOVERY_SOURCES, listProspects, statusCounts, getProspect, addProspect, importProspects, importMetrics, setStatus as setProspectStatus, updateProspect, addNote as addProspectNote, getNotes as getProspectNotes, removeProspect } from '../lib/prospects.js';
import { runClassify, discoverFromUrl, buildDorks, extractDomains, enrichMetrics } from '../lib/prospecting/index.js';
...
<script>(function(){var R=document.getElementById('gencard-${id}');if(!R)return;var btns=R.querySelectorAll('.genbtn');var one=R.querySelector('.gm-one'),lst=R.querySelector('.gm-list'),cnt=R.querySelector('.gm-count');var kw=R.querySelector('[name=keyword]'),list=R.querySelector('[name=list]'),count=R.querySelector('[name=count]'),clbl=R.querySelector('.count-label');var cli=R.querySelector('input[name=backend][value=cli]'),cf=R.querySelector('[name=confirm]');function newsOf(){var o=list&&list.options[list.selectedIndex];return o?parseInt(o.getAttribute('data-new'),10)||0:0;}function mode(){var c=R.querySelector('input[name=gensrc]:checked');return c?c.value:'one';}function valid(m){if(!R.querySelector('input[name=backend]:checked'))return false;if(cf&&!cf.checked)return false;if(m==='list')return !!(list&&list.value)&&!!(count&&parseInt(count.value,10)>0);if(m==='batch')return !!(count&&parseInt(count.value,10)>0);return true;}function validate(){var m=mode();btns.forEach(function(b){if(b.getAttribute('data-mode')===m)b.disabled=!valid(m);});}function apply(m){one.classList.toggle('d-none',m!=='one');lst.classList.toggle('d-none',m!=='list');cnt.classList.toggle('d-none',m==='one');if(kw)kw.disabled=m!=='one';if(list)list.disabled=m!=='list';if(count)count.disabled=m==='one';if(m==='list'){clbl.textContent='Сколько (из новых)';var n=newsOf();count.max=n>0?n:1;if(!count.value||parseInt(count.value,10)>n)count.value=n||1;}else if(m==='batch'){clbl.textContent='Статей';count.removeAttribute('max');}if(cli){cli.disabled=(m==='batch');if(m==='batch'&&cli.checked)cli.checked=false;}btns.forEach(function(b){var on=b.getAttribute('data-mode')===m;b.classList.toggle('d-none',!on);b.disabled=!on;});validate();}R.querySelectorAll('input[name=gensrc]').forEach(function(r){r.addEventListener('change',function(){if(r.checked)apply(r.value);});});R.querySelectorAll('input[name=backend]').forEach(function(r){r.addEventListener('change',validate);});if(cf)cf.addEventListener('change',validate);if(count)count.addEventListener('input',validate);if(list)list.addEventListener('change',function(){apply('list');});var c=R.querySelector('input[name=gensrc]:checked');apply(c?c.value:'one');})();</script></form>`
...
<td><div class="d-flex gap-1 flex-wrap">${it.article_id ? '' : `<form method="post" action="/lists/items/${it.id}/generate" class="d-flex gap-1"><select name="prompt" class="form-select form-select-sm" style="width:auto">${promptOpts}</select>${backendSel}<button class="btn btn-sm btn-primary" onclick="return confirm('Сгенерировать статью под ключ?')">Сген.</button></form>`}<form method="post" action="/lists/items/${it.id}/status"><input type="hidden" name="status" value="winner"><button class="btn btn-sm btn-outline-success" title="зашёл"><i class="ti ti-check"></i></button></form><form method="post" action="/lists/items/${it.id}/status"><input type="hidden" name="status" value="loser"><button class="btn btn-sm btn-outline-danger" title="не зашёл"><i class="ti ti-x"></i></button></form><form method="post" action="/lists/items/${it.id}/status"><input type="hidden" name="status" value="skip"><button class="btn btn-sm btn-outline-secondary" title="пропуск"><i class="ti ti-player-skip-forward"></i></button></form><form method="post" action="/lists/items/${it.id}/remove" onsubmit="return confirm('Убрать из списка?')"><button class="btn btn-sm btn-outline-secondary" title="убрать из списка"><i class="ti ti-trash"></i></button></form></div></td></tr>`,
  ...
  app.post('/lists/bulk-generate', async (req, reply) => {
    ...
    const items = db.prepare("SELECT * FROM kw_list_items WHERE list_id = ? AND status = 'new' ORDER BY score DESC LIMIT ?").all(list.id, n);
    if (!items.length) return reply.redirect(`/lists/${list.id}?msg=${encodeURIComponent('В списке нет новых ключей для теста')}`);
    const jobId = createJob('generate', { siteId: prompt.site_id });
...
<div class="card-footer">${proxyImport}<div class="text-secondary small mt-2">Резидентные ротационные: выдаются по принципу «давно не использованная — первой» (реюз после простоя). При импорте почт без своей прокси берётся свободная из пула указанной страны.</div></div></div>`;
  ...
  const pBadge = (s) => {
    const m = PROSPECT_STATUSES[s] || { label: s, badge: 'bg-secondary' };
    return `<span class="badge ${m.badge} text-white">${esc(m.label)}</span>`;
  };
  const flagIcon = (v) =>
    v === null || v === undefined
  ...
  const flagSel = (name, val) =>
    `<select name="${name}" class="form-select form-select-sm">
<option value=""${val === null || val === undefined ? ' selected' : ''}>—</option>
<option value="1"${val === 1 ? ' selected' : ''}>да</option>
<option value="0"${val === 0 ? ' selected' : ''}>нет</option></select>`;

  app.get('/prospects', async (req, reply) => {
    const status = req.query.status || 'all';
    const sort = req.query.sort === 'score' ? 'score' : 'new';
    const counts = statusCounts(db);
    const rows = listProspects(db, { status, sort });
    const sites = db.prepare('SELECT id, name FROM sites').all();
    const siteName = (sid) => sites.find((x) => x.id === sid)?.name || `#${sid}`;
    const noteCounts = Object.fromEntries(
      db.prepare('SELECT prospect_id, COUNT(*) c FROM prospect_notes GROUP BY prospect_id').all().map((r) => [r.prospect_id, r.c]),
    );

    // Чипы-фильтры по статусам (+ all).
    const chip = (key, label, n) =>
      `<a href="/prospects?status=${key}" class="btn btn-sm ${status === key ? 'btn-primary' : 'btn-outline-secondary'}">${esc(label)} <span class="badge bg-secondary text-white ms-1">${n}</span></a>`;
    const chips = [
      chip('all', 'все', counts.all),
      ...Object.entries(PROSPECT_STATUSES).map(([k, v]) => chip(k, v.label, counts[k] || 0)),
    ].join(' ');
    // Переключатель сортировки (новизна / мощность), сохраняет текущий статус-фильтр.
    const sortLink = (key, label) =>
      `<a href="/prospects?status=${status}&sort=${key}" class="btn btn-sm ${sort === key ? 'btn-primary' : 'btn-outline-secondary'}">${esc(label)}</a>`;
    const sortBar = `<div class="d-flex align-items-center gap-1 mb-3"><span class="text-secondary small me-1">сортировка:</span>${sortLink('new', 'новизна')}${sortLink('score', 'мощность')}</div>`;

    const tr = rows
      .map((p) => {
        const dr = p.authority == null ? '<span class="text-secondary">—</span>' : esc(p.authority);
        const tf = p.traffic == null ? '<span class="text-secondary">—</span>' : esc(p.traffic.toLocaleString('ru-RU'));
        const link = p.adopted_site_id
          ? `<a href="/sites/${p.adopted_site_id}" title="боевой сайт">${esc(siteName(p.adopted_site_id))}</a>`
          : p.adapter
            ? `<span class="text-secondary small">${esc(p.adapter)}</span>`
            : '<span class="text-secondary">—</span>';
        const nc = noteCounts[p.id] || 0;
        return `<tr>
<td>${p.id}</td>
<td><a href="/prospects/${p.id}"><b>${esc(p.domain)}</b></a> <a href="https://${esc(p.domain)}" target="_blank" rel="noopener" class="text-secondary" title="открыть"><i class="ti ti-external-link"></i></a>${p.name ? `<div class="text-secondary small">${esc(p.name)}</div>` : ''}</td>
<td class="text-end">${p.score == null ? '<span class="text-secondary">—</span>' : `<b>${esc(p.score)}</b>`}</td>
<td>${esc(p.engine)}</td>
<td>${esc(p.country || '—')}</td>
<td class="text-end">${dr}</td>
<td class="text-end">${tf}</td>
<td class="text-center" title="регистрация">${flagIcon(p.has_register)}</td>
<td class="text-center" title="форма статьи (UGC)">${flagIcon(p.has_ugc_form)}</td>
<td class="text-center" title="dofollow">${flagIcon(p.dofollow)}</td>
<td>${pBadge(p.status)}</td>
<td>${link}</td>
<td>${nc ? `<a href="/prospects/${p.id}#notes" class="text-secondary"><i class="ti ti-message"></i> ${nc}</a>` : '<span class="text-secondary">—</span>'}</td>
</tr>`;
      })
      .join('');

    const head = ['id', 'домен', 'скор', 'движок', 'стр.', 'DR', 'трафик', 'R', 'U', 'F', 'статус', 'сайт/адаптер', 'зам.'];
    const engOpts = PROSPECT_ENGINES.map((e) => `<option value="${e}">${esc(e)}</option>`).join('');
    const srcOpts = DISCOVERY_SOURCES.map((s) => `<option value="${s}">${esc(s)}</option>`).join('');
    const addForm = `<form method="post" action="/prospects" class="row g-2 align-items-end">
<div class="col-auto"><label class="form-label">Домен</label><input name="domain" class="form-control" placeholder="example.de" required></div>
<div class="col"><label class="form-label">Название</label><input name="name" class="form-control" placeholder="опц."></div>
<div class="col-auto"><label class="form-label">Движок</label><select name="engine" class="form-select">${engOpts}</select></div>
<div class="col-auto"><label class="form-label">Страна</label><input name="country" class="form-control" value="de" style="width:5rem"></div>
<div class="col-auto"><label class="form-label">Источник</label><select name="discovery_source" class="form-select">${srcOpts}</select></div>
<div class="col-auto"><button type="submit" class="btn btn-primary">Добавить</button></div></form>`;
    const importForm = `<details><summary class="btn btn-outline-secondary btn-sm">Массовый импорт</summary>
<div class="mt-2"><form method="post" action="/prospects/import">
<div class="mb-2 d-flex gap-2 align-items-center"><span class="small text-secondary">движок:</span><select name="engine" class="form-select form-select-sm" style="width:auto">${engOpts}</select><span class="small text-secondary">страна:</span><input name="country" class="form-control form-control-sm" value="de" style="width:5rem"><span class="small text-secondary">источник:</span><select name="discovery_source" class="form-select form-select-sm" style="width:auto">${srcOpts}</select></div>
<textarea name="text" class="form-control mono" rows="6" placeholder="по строке: домен  (или) домен, Название  (или) домен | Название&#10;myheimat.de, myHeimat&#10;lokalkompass.de | Lokalkompass"></textarea>
<button type="submit" class="btn btn-primary mt-2">Импортировать</button></form></div></details>`;
    const seedBtn = `<form method="post" action="/prospects/seed" class="d-inline"><button class="btn btn-outline-secondary btn-sm" title="внести найденные PEIQ-кандидаты (идемпотентно)"><i class="ti ti-seeding"></i> Засеять PEIQ</button></form>`;
    const enrichBtn = `<form method="post" action="/prospects/enrich-metrics" class="d-inline" onsubmit="return confirm('Запросить метрики SEMrush (Authority Score + трафик) для кандидатов без метрик? Тратит API-юниты.')"><button class="btn btn-outline-secondary btn-sm" title="SEMrush API: Authority Score + трафик → скор (для кандидатов без метрик)"><i class="ti ti-chart-arrows"></i> Метрики SEMrush</button></form>`;
    const footer = `${addForm}<div class="mt-2 d-flex gap-2 align-items-center">${importForm}${seedBtn}${enrichBtn}</div>`;

    // Карточка дискавери: дорки PEIQ + извлечение доменов из текста + скачивание страницы-источника.
    const dorksHtml = buildDorks('').map((q) => `<code>${esc(q)}</code>`).join('<br>');
    const discoverCard = `<div class="card mb-3"><div class="card-header"><h3 class="card-title"><i class="ti ti-world-search"></i> Дискавери</h3></div><div class="card-body">
<details><summary class="btn btn-outline-secondary btn-sm">Google-дорки PEIQ</summary><div class="mt-2 small">${dorksHtml}<div class="text-secondary mt-1">Прогони в Google/SEMrush, выдачу вставь в поле ниже.</div></div></details>
<form method="post" action="/prospects/extract" class="mt-3">
<label class="form-label">Извлечь домены из текста (выдача SERP / HTML / список)</label>
<div class="d-flex gap-2 mb-2 align-items-center"><span class="small text-secondary">движок:</span><select name="engine" class="form-select form-select-sm" style="width:auto">${engOpts}</select><span class="small text-secondary">страна:</span><input name="country" class="form-control form-control-sm" value="de" style="width:5rem"></div>
<textarea name="text" class="form-control mono" rows="5" placeholder="вставь выдачу/HTML/список — домены извлекутся, дедупнутся, мусор (соцсети/CDN/peiq.de) отсеется"></textarea>
<button class="btn btn-primary mt-2">Извлечь и добавить</button></form>
<form method="post" action="/prospects/discover" class="mt-3 row g-2 align-items-end">
<div class="col"><label class="form-label">Дискавери по URL (скачать страницу → извлечь домены)</label><input name="url" class="form-control" placeholder="https://news.peiq.de/c-kunden"></div>
<div class="col-auto"><label class="form-label">Страна</label><input name="country" class="form-control" value="de" style="width:5rem"></div>
<div class="col-auto"><button class="btn btn-outline-primary">Скачать и извлечь</button></div></form>
<form method="post" action="/prospects/import-metrics" class="mt-3">
<label class="form-label">Метрики мощности (SEMrush bulk-экспорт / CSV) — обновит authority и трафик по домену, пересчитает скор</label>
<textarea name="text" class="form-control mono" rows="5" placeholder="CSV или вставка с заголовком (Domain, Authority Score, Organic Traffic) либо по порядку:&#10;myheimat.de,72,1200000&#10;lokalkompass.de;55;340000"></textarea>
<button class="btn btn-outline-primary mt-2">Импортировать метрики</button></form>
</div></div>`;

    const note =
      '<div class="text-secondary small mb-2">Каталог площадок-доноров. Движок meinbezirk.at — PEIQ; кандидаты с тем же движком переиспользуют существующий адаптер. Колонки: <b>R</b> — регистрация, <b>U</b> — форма статьи (UGC), <b>F</b> — dofollow.</div>';
    const inner =
      note +
      `<div class="mb-3 d-flex flex-wrap gap-1">${chips}</div>` +
      sortBar +
      discoverCard +
      tableCard(`<i class="ti ti-radar"></i> Площадки (${rows.length})`, head, tr, null, footer);
    reply.type('text/html').send(page('/prospects', 'Площадки', inner, { flash: flash(req.query) }));
  });

  app.post('/prospects', async (req, reply) => {
    const b = req.body;
    try {
      const id = addProspect(db, { domain: b.domain, name: b.name, engine: b.engine, country: b.country, discovery_source: b.discovery_source });
      reply.redirect(`/prospects/${id}`);
    } catch (e) {
      reply.redirect(`/prospects?msg=${encodeURIComponent('Ошибка: ' + e.message)}`);
    }
  });

  app.post('/prospects/import', async (req, reply) => {
    const r = importProspects(db, req.body.text, { engine: req.body.engine, country: req.body.country, discovery_source: req.body.discovery_source });
    reply.redirect(`/prospects?msg=${encodeURIComponent(`Импорт: добавлено ${r.added}, дублей ${r.skipped}, ошибок ${r.errors.length}`)}`);
  });

  app.post('/prospects/seed', async (req, reply) => {
    const { seedProspects } = await import('../lib/prospects.js');
    const n = seedProspects(db);
    reply.redirect(`/prospects?msg=${encodeURIComponent(`Засеяно новых: ${n}`)}`);
  });

  app.get('/prospects/:id', async (req, reply) => {
    const id = Number(req.params.id);
    const p = getProspect(db, id);
    if (!p) return reply.code(404).type('text/html').send(page('/prospects', 'Площадка', '<div class="alert alert-danger">Кандидат не найден.</div>'));
    const sites = db.prepare('SELECT id, name FROM sites').all();
    const notes = getProspectNotes(db, id);

    const engOpts = PROSPECT_ENGINES.map((e) => `<option value="${e}"${p.engine === e ? ' selected' : ''}>${esc(e)}</option>`).join('');
    const srcOpts = DISCOVERY_SOURCES.map((s) => `<option value="${s}"${p.discovery_source === s ? ' selected' : ''}>${esc(s)}</option>`).join('');
    const siteOpts =
      '<option value="">— не привязан —</option>' +
      sites.map((s) => `<option value="${s.id}"${p.adopted_site_id === s.id ? ' selected' : ''}>${esc(s.name)} (#${s.id})</option>`).join('');

    const editForm = `<form method="post" action="/prospects/${id}/edit" class="row g-2">
<div class="col-md-6"><label class="form-label">Название</label><input name="name" class="form-control" value="${esc(p.name || '')}"></div>
<div class="col-md-3"><label class="form-label">Движок</label><select name="engine" class="form-select">${engOpts}</select></div>
<div class="col-md-3"><label class="form-label">Страна</label><input name="country" class="form-control" value="${esc(p.country || '')}"></div>
<div class="col-md-6"><label class="form-label">URL (форма/пример)</label><input name="url" class="form-control" value="${esc(p.url || '')}"></div>
<div class="col-md-3"><label class="form-label">Источник</label><select name="discovery_source" class="form-select">${srcOpts}</select></div>
<div class="col-md-3"><label class="form-label">DR/Authority</label><input name="authority" class="form-control" value="${p.authority ?? ''}"></div>
<div class="col-md-3"><label class="form-label">Трафик/мес</label><input name="traffic" class="form-control" value="${p.traffic ?? ''}"></div>
<div class="col-md-3"><label class="form-label">Регистрация</label>${flagSel('has_register', p.has_register)}</div>
<div class="col-md-3"><label class="form-label">Форма статьи (UGC)</label>${flagSel('has_ugc_form', p.has_ugc_form)}</div>
<div class="col-md-3"><label class="form-label">dofollow</label>${flagSel('dofollow', p.dofollow)}</div>
<div class="col-md-3"><label class="form-label">Адаптер (lib/sites)</label><input name="adapter" class="form-control" value="${esc(p.adapter || '')}" placeholder="напр. myheimat"></div>
<div class="col-md-6"><label class="form-label">Боевой сайт</label><select name="adopted_site_id" class="form-select">${siteOpts}</select></div>
<div class="col-12"><button class="btn btn-primary">Сохранить</button></div></form>`;

    const statOpts = Object.entries(PROSPECT_STATUSES)
      .map(([k, v]) => `<option value="${k}"${p.status === k ? ' selected' : ''}>${esc(v.label)}</option>`)
      .join('');
    const statusForm = `<form method="post" action="/prospects/${id}/status" class="row g-2 align-items-end">
<div class="col-auto"><label class="form-label">Статус</label><select name="status" class="form-select">${statOpts}</select></div>
<div class="col"><label class="form-label">Причина/комментарий (опц.)</label><input name="reason" class="form-control" value="${esc(p.reject_reason || '')}"></div>
<div class="col-auto"><button class="btn btn-primary">Применить</button></div></form>`;

    const kindIcon = { note: 'message', status: 'flag', system: 'robot' };
    const notesList = notes.length
      ? notes
          .map(
            (n) =>
              `<div class="d-flex gap-2 mb-2"><span class="text-secondary"><i class="ti ti-${kindIcon[n.kind] || 'point'}"></i></span><div><div>${esc(n.text)}</div><div class="text-secondary small">${esc(n.ts)}</div></div></div>`,
          )
          .join('')
      : '<div class="text-secondary">заметок нет</div>';
    const noteForm = `<form method="post" action="/prospects/${id}/note" class="mt-2"><div class="input-group"><input name="text" class="form-control" placeholder="комментарий…" required><button class="btn btn-outline-primary">Добавить</button></div></form>`;

    const header = `<div class="d-flex align-items-center gap-2 mb-3">
<h2 class="m-0"><a href="https://${esc(p.domain)}" target="_blank" rel="noopener" class="text-reset">${esc(p.domain)} <i class="ti ti-external-link text-secondary"></i></a></h2>
${pBadge(p.status)}
<form method="post" action="/prospects/${id}/classify" class="ms-auto" title="скачать сайт и определить движок/UGC/dofollow"><button class="btn btn-sm btn-outline-primary"><i class="ti ti-radar-2"></i> Классифицировать</button></form>
<a href="/prospects" class="btn btn-sm btn-outline-secondary">← к списку</a></div>`;
    const delForm = `<form method="post" action="/prospects/${id}/delete" onsubmit="return confirm('Удалить кандидата ${esc(p.domain)} из каталога?')"><button class="btn btn-outline-danger btn-sm"><i class="ti ti-trash"></i> Удалить из каталога</button></form>`;

    const inner =
      header +
      card('<i class="ti ti-flag"></i> Статус', statusForm) +
      card('<i class="ti ti-adjustments"></i> Данные и метрики', editForm) +
      card('<i class="ti ti-message"></i> Журнал', notesList + noteForm, 'notes') +
      delForm;
    reply.type('text/html').send(page('/prospects', `Площадка ${p.domain}`, inner, { flash: flash(req.query) }));
  });

  app.post('/prospects/:id/edit', async (req, reply) => {
    const id = Number(req.params.id);
    try {
      updateProspect(db, id, req.body);
      reply.redirect(`/prospects/${id}?msg=${encodeURIComponent('Сохранено')}`);
    } catch (e) {
      reply.redirect(`/prospects/${id}?msg=${encodeURIComponent('Ошибка: ' + e.message)}`);
    }
  });

  app.post('/prospects/:id/status', async (req, reply) => {
    const id = Number(req.params.id);
    try {
      setProspectStatus(db, id, req.body.status, req.body.reason);
      reply.redirect(`/prospects/${id}?msg=${encodeURIComponent('Статус обновлён')}`);
    } catch (e) {
      reply.redirect(`/prospects/${id}?msg=${encodeURIComponent('Ошибка: ' + e.message)}`);
    }
  });

  app.post('/prospects/:id/note', async (req, reply) => {
    const id = Number(req.params.id);
    addProspectNote(db, id, req.body.text, 'note');
    reply.redirect(`/prospects/${id}#notes`);
  });

  app.post('/prospects/:id/delete', async (req, reply) => {
    removeProspect(db, Number(req.params.id));
    reply.redirect(`/prospects?msg=${encodeURIComponent('Кандидат удалён')}`);
  });

  // Классификация: скачать сайт → движок/UGC/dofollow → сохранить + заметка (+ авто-qualify).
  app.post('/prospects/:id/classify', async (req, reply) => {
    const id = Number(req.params.id);
    try {
      const r = await runClassify(db, id);
      const msg = r.ok
        ? `Классификация: движок=${r.engine || 'не PEIQ'}, регистрация=${r.has_register ? 'да' : 'нет'}, форма=${r.has_ugc_form ? 'да' : 'нет'}, dofollow=${r.dofollow == null ? '—' : r.dofollow ? 'да' : 'нет'}`
        : `Классификация не удалась: ${r.error}`;
      reply.redirect(`/prospects/${id}?msg=${encodeURIComponent(msg)}`);
    } catch (e) {
      reply.redirect(`/prospects/${id}?msg=${encodeURIComponent('Ошибка: ' + e.message)}`);
    }
  });

  // Извлечь домены из вставленного текста (выдача/HTML) → импорт кандидатов.
  app.post('/prospects/extract', async (req, reply) => {
    const domains = extractDomains(req.body.text || '');
    const r = importProspects(db, domains.join('\n'), { engine: req.body.engine, country: req.body.country, discovery_source: 'serp' });
    reply.redirect(`/prospects?msg=${encodeURIComponent(`Извлечено доменов: ${domains.length}; добавлено ${r.added}, дублей ${r.skipped}`)}`);
  });

  // Автообогащение метрик через SEMrush API (Authority Score + трафик) для кандидатов без метрик.
  app.post('/prospects/enrich-metrics', async (req, reply) => {
    try {
      const r = await enrichMetrics(db, { limit: 50 });
      reply.redirect(`/prospects?sort=score&msg=${encodeURIComponent(`SEMrush[${r.account}]: обогащено ${r.enriched}${r.unitsLeft != null ? `, юнитов ${r.unitsLeft}` : ''}`)}`);
    } catch (e) {
      reply.redirect(`/prospects?msg=${encodeURIComponent('Метрики SEMrush: ' + e.message)}`);
    }
  });

  // Импорт метрик мощности (SEMrush bulk/CSV) → обновить authority/traffic существующих + пересчёт скора.
  app.post('/prospects/import-metrics', async (req, reply) => {
    const r = importMetrics(db, req.body.text || '', { source: 'semrush' });
    reply.redirect(`/prospects?sort=score&msg=${encodeURIComponent(`Метрики: обновлено ${r.updated}, не найдено в базе ${r.unmatched}, ошибок ${r.errors.length}`)}`);
  });

  // Дискавери по URL: скачать страницу-источник → извлечь домены → импорт.
  app.post('/prospects/discover', async (req, reply) => {
    const url = String(req.body.url || '').trim();
    if (!url) return reply.redirect(`/prospects?msg=${encodeURIComponent('Укажи URL')}`);
    try {
      const r = await discoverFromUrl(db, url, { country: req.body.country, discovery_source: 'serp' });
      const msg = r.ok ? `Дискавери: найдено ${r.domains.length}, добавлено ${r.added}, дублей ${r.skipped}` : `Дискавери не удалась: ${r.error}`;
      reply.redirect(`/prospects?msg=${encodeURIComponent(msg)}`);
    } catch (e) {
      reply.redirect(`/prospects?msg=${encodeURIComponent('Ошибка: ' + e.message)}`);
    }
  });
